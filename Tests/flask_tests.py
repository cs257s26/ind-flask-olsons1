...
class Tests(unittest.TestCase):
    ...
    def test_sightings_path(self):
        response = self.client.get('/sightingslocationall/2018/American Crow (Corvus brachyrhynchos) /')
    ...
    def test_sightings_file_not_found(self):
        response = self.client.get('/sightingslocationall/1900/AHHHHH/')
        self.assertEqual(response.status_code, 200)

        self.assertIn(b'The file was not found...', response.data)

    def test_sightings_zero_count(self):
        response = self.client.get('/sightingslocationall/2018/Barn Swallow (Hirundo rustica) /')
        self.assertEqual(response.status_code, 200)
    ...
    def test_sightings_non_year(self):
        response = self.client.get('/sightingslocationall/notyear/Bird/')
        self.assertEqual(response.status_code, 200)
